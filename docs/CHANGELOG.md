...
- [新功能] Strategy Lab V0.1 新增 Replay Evidence Manifest Differential Foundation（`src/services/strategy_lab/replay_evidence_manifest.py`：`EvidenceTimeEnvelope`/`TimeQuality`/`ReplayEvidenceManifest`/`ReplayManifestFieldDiff`/`ReplayManifestDifferential`/`ReplayDeterminismCheck`，入口 `diff_replay_manifests`/`check_replay_determinism`/`require_derived_output_distinct`），为一份被捕获的重放证据（fixture/capture）提供不可变、内容寻址的身份，并对两份此类 manifest 做纯差分比较；回答的是"这份证据的受治理身份是否变化、具体是哪个字段变的"，绝不回答"策略是否盈利"，也绝不重放/重新执行任何东西。刻意不是：第二个重放引擎（只持有并比较声明事实，从不抓取/重跑）；第二个实验权威（`experiment_id` 仅为指向既有 Experiment Registry `experiment_governance.ExperimentManifest.experiment_id` 的外部引用，不重新实现谱系/根不变量/身份冲突裁决，那仍完全归 `experiment_governance` 所有）；provider 运行时或 Currentness 运行时（`provider`/`provider_endpoint`/`calendar_source` 为不透明的调用方声明字符串，本模块不调用任何 provider、不定义"当前"、不解析交易日历）；Shadow/CORE/LIVE 路径或 broker/order 路径（无任何执行面）；OOS Consumption Ledger 自身幂等重放语义（`OOSClaimStatus.IDEMPOTENT_REPLAY`/`OOSBurnStatus.IDEMPOTENT_REPLAY`）的替代品——本模块的"重放"指证据捕获的可复现性，是与 Ledger 操作幂等性完全不同的概念，从不读写 Ledger 状态，既有 Strategy Lab 重放/OOS 语义保持权威且不受影响。UNKNOWN 恒为 UNKNOWN：除 `experiment_id`/`fixture_id`/`capture_id`/`schema_version` 外全部字段可选，`None` 表示"该事实未知"而非"该事实为空"，从不被强制转换、默认或静默升级为确定值，补齐此前未知字段本身就是一次受治理身份变更；`event_time`/`publication_time`/`observed_at` 三者各自携带独立的 `TimeQuality`（`UNKNOWN`/`EXACT`/`APPROXIMATE`），值为 `None` 时质量必须为 `UNKNOWN`、值存在时质量禁止为 `UNKNOWN`，二者不一致直接拒绝。三个时间字段刻意从不互相派生——`EvidenceTimeEnvelope` 不存在任何跨字段默认值或回退分支，这是结构性保证；但模块刻意不额外禁止三者数值相等（例如某 provider 在同一瞬间同时盖 `observed_at`与`publication_time`属于合法真实事实而非本模块的派生行为），强行禁止重合值会让真实存在的捕获场景无法表达而不带来任何安全收益。`manifest_hash` 为 `SHA256(canonical(schema_version + 全部 26 个受治理字段))`，字段清单以显式硬编码 `GOVERNED_FIELD_NAMES` 常量维护并由结构测试校验其与实际 canonical payload 一致，`created_at` 与 `experiment_governance.ExperimentManifest` 一样以结构方式排除（从不传入 canonicalizer，而非事后从整体中剔除）；`manifest_hash` 本身为只读派生属性，无对应可赋值字段，调用方无法伪造。`diff_replay_manifests` 对两份 manifest 的全部受治理字段做逐字段比较并汇总 `changed_fields`，`identity_changed` 直接读取 `manifest_hash` 是否相等——二者不可能因为额外运行时校验而彼此矛盾，因为 `identity_changed` 与逐字段比较依据的是完全相同的受治理字段集合。`check_replay_determinism` 是"相同受治理 manifest + 相同声明证据 → 相同声明输出"的纯校验：manifest_hash 不同的两份 manifest 彼此不承担一致性义务；manifest_hash 相同但调用方声明的输出摘要不同才判定为不一致——本函数只比较调用方提供的摘要，绝不执行、抓取或重新生成任何输出，因此不构成第二个重放引擎。`require_derived_output_distinct` 是独立的辅助校验：拒绝 `derived_output_ref` 与 `raw_evidence_ref` 完全相等，防止派生产物（计算得到的特征、渲染出的报告、重新编码的副本等）冒充其原始证据本身；"派生输出"未被纳入最小 manifest 字段清单，因为它属于调用方自行拥有的存储，而非证据身份 schema 本身的一部分。模块为仅依赖 stdlib 与既有 Temporal Contract（`canonical_utc_datetime`/`canonical_utc_text`）的纯计算叶子节点，不引入 SQLAlchemy、repositories、数据库、trading calendar、任何 live provider 或执行/broker 路径，也未修改 `experiment_governance.py`/`oos_consumption.py`/`temporal_contract.py` 任何既有代码。
