...
class ExecutionStore:
    ...
    def intents(self) -> list[OrderIntent]:
        ...
        for (raw,) in self.connection.execute("SELECT payload FROM intents ORDER BY intent_id"):
            item = json.loads(raw); result.append(OrderIntent(item["intent_id"], item["symbol"], Side(item["side"]), OrderType(item["order_type"]), Decimal(item["qty"]), Decimal(item["limit_price"]) if item["limit_price"] else None, Decimal(item["stop_price"]) if item.get("stop_price") else None, Decimal(item["max_slippage"]) if item.get("max_slippage") else None, Decimal(item["invalidation"]) if item.get("invalidation") else None, Decimal(item["risk_budget_r"]) if item.get("risk_budget_r") else None, datetime.fromisoformat(item["valid_until"]) if item.get("valid_until") else None, strategy_id=item["strategy_id"], evidence_snapshot_id=item["evidence_snapshot_id"], account_target=item["account_target"], broker_target=item["broker_target"], allowed_session=item["allowed_session"]))
        return result
